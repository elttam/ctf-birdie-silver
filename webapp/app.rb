...
post "/login" do
  ...
  redirect to("/login?error=Too long") if username.size > 20

  # intermediate variable used because cookies hands urlencoded value back
  cookies[:session_data] = session_data = JSON.dump({"username" => username})
...
<html>
 ...
 <body>
  <h1><font color="Silver"><%= h @title %></font></h1>
  <p><%= yield %></p>
 </body>
</html>
...
<% else %>
Ah, just a pheasant user, I see. Find a way to login as admin.
<% end %>
<br /><br />
<img src="silver-bird.jpg" />
